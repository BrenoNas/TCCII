...
void MultirotorPawnSimApi::onCollision(UPrimitiveComponent* MyComp, AActor* Other, UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
    const CollisionInfo& collision_info = getCollisionInfo();
    collision_response = multirotor_physics_body_->getCollisionResponseInfo();

    collision_count_aux = collision_response.collision_count_non_resting;
    PawnSimApi::onCollision(MyComp, Other, OtherComp, bSelfMoved, HitLocation, HitNormal, NormalImpulse, Hit);
}
